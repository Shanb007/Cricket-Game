...
public class InningServiceImpl implements InningsService {
    ...
    public ArrayList<String> InitiateInnings(Matches matches, Team bat, Team ball, int overs, String inning) throws SQLException {
        ArrayList<String> output = new ArrayList<>();
        this.match = matches;
        this.OnBatTeam = bat;
        this.OnBallTeam = ball;
        this.overs = overs;
        teamRepository.setScoreBoard(match.getMatchID(),OnBatTeam.getTeamID());
    ...
    public String addFirstTwoBatsman(int matchID, int BatTeamID, String Bat1, String Bat2) throws SQLException {
        ...
        playerRepository.setDataPlayerMatchDetailsDB(playerRepository.getPlayerIdByTeamIdAndPlayerName(BatTeamID,Bat1),matchID,onFieldBat.get(currentOnStrike));
        playerRepository.setDataPlayerMatchDetailsDB(playerRepository.getPlayerIdByTeamIdAndPlayerName(BatTeamID,Bat1),matchID,onFieldBat.get(currentNotOnStrike));
    ...
    public String addBatPlayerAfterWicket(int matchID, int teamID, String player) throws SQLException {
        ...
        playerRepository.setDataPlayerMatchDetailsDB(playerRepository.getPlayerIdByTeamIdAndPlayerName(teamID,player),matchID,batAfterWicket);
    ...
    public String overBy(int matchID, int BallTeamID, String player) throws SQLException {
        ...
        if(!playerRepository.checkIfBowlerAlreadyInMatch(currentBowlerID,matchID)){
            ...
            bowler.setDidBall("Y");
            playerRepository.setDataPlayerMatchDetailsDB(currentBowlerID,matchID,bowler);
        }
        else {
            //get the bowler object from db.
            bowler = playerRepository.getNextOverBowlerRequested(matchID, BallTeamID, player);
        }
        return "Next Over will be bowled by: "+bowler.getName();
    }
    ...
    public String playOver(int matchID, String inning, int BatTeamID, int BallTeamID, int ballingPlayerID) throws SQLException {
        ...
        if(ballRepository.getLastBallPlayedOutcome(matchID,BatTeamID).equals("W") && ballRepository.getLastBallNumberPlayed(matchID,BatTeamID)%6 != 0){
            ...
            startBall = ballRepository.getLastBallNumberPlayed(matchID,BatTeamID);
        }
...
//        System.out.print("\nEnter the " + OnBatTeam.totalWicketsGone() + " wicket down player, to bat: ");
        ...
        playerRepository.updateBowlerStatsInDB(matchID,BallTeamID,bowler);
        playerRepository.updateOnFieldBatsmanStatsIDB(matchID,OnBatTeam.getTeamID(),onFieldBat);
        return "It's a wicket at "+ball+" ball of this over. "+onFieldBat.get(currentOnStrike)+ " has to make hs way back to pavilion. Kindly Enter the next batsman.";
